...
<div style="max-width: 500px; margin: 4rem auto;">
    <div style="background: white; padding: 2.5rem; border-radius: var(--radius); border: 1px solid var(--border); box-shadow: var(--shadow);">
        <h2 style="font-size: 1.875rem; font-weight: 800; margin-bottom: 0.5rem; color: var(--primary);">Criar conta</h2>
        <p style="color: var(--text-muted); margin-bottom: 2rem;">Junte-se à nossa comunidade de T-Shirts personalizadas.</p>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.25rem;">
                ...
                <div>
                    <label for="password_confirmation" style="display: block; font-size: 0.875rem; font-weight: 600; margin-bottom: 0.5rem;">Confirmar</label>
            ...
            @error('password')
                <span style="color: #ef4444; font-size: 0.75rem; margin-top: -0.75rem; margin-bottom: 1.25rem; display: block;">{{ $message }}</span>
            @enderror

            <button type="submit" class="btn btn-primary" style="width: 100%; padding: 0.875rem; margin-top: 1rem;">Criar Conta</button>
        </form>

        <p style="text-align: center; margin-top: 2rem; font-size: 0.875rem; color: var(--text-muted);">
            Já tem conta? <a href="{{ route('login') }}" style="color: var(--secondary); text-decoration: none; font-weight: 600;">Faça Login</a>
        </p>
    </div>
</div>
@endsection
